...
if "MieleCrypto" not in sys.modules:
    ...
    _mock_mc.MieleProvisioningInfo = _mock_cls  # type: ignore[attr-defined]
...
class TestPromptYesNo(unittest.TestCase):
    ...
    def test_yes_full(self, _: Any) -> None:
        self.assertTrue(setup.prompt_yes_no("q?"))

    @patch("builtins.input", return_value="n")
    def test_no(self, _: Any) -> None:
        self.assertFalse(setup.prompt_yes_no("q?"))

    @patch("builtins.input", return_value="NO")
    def test_no_case_insensitive(self, _: Any) -> None:
        self.assertFalse(setup.prompt_yes_no("q?"))


class TestBanner(unittest.TestCase):
    @patch("builtins.print")
    def test_banner_prints_equals_lines(self, mock_print: MagicMock) -> None:
        setup.banner("Test")
...
class TestProbeHost(unittest.TestCase):
    @patch("socket.socket")
    def test_returns_ip_on_port80(self, mock_sock_cls: MagicMock) -> None:
        mock_sock = MagicMock()
    ...
    @patch("socket.socket")
    def test_returns_none_when_both_fail(self, mock_sock_cls: MagicMock) -> None:
        mock_sock = MagicMock()
    ...
    @patch("socket.socket")
    def test_returns_ip_on_port443_fallback(self, mock_sock_cls: MagicMock) -> None:
        mock_sock = MagicMock()
...
class TestScanSubnet(unittest.TestCase):
    ...
    @patch("miele_device_setup.get_local_ip", return_value="192.168.1.50")
    def test_returns_empty_when_no_hosts_respond(self, *_: Any) -> None:
        self.assertEqual(setup.scan_subnet(), [])
...
class TestDiscoverMieleMdns(unittest.TestCase):
    ...
    def test_returns_empty_without_zeroconf(self) -> None:
    ...
    @patch("time.sleep")
    def test_returns_found_devices(self, mock_sleep: MagicMock, mock_browser_cls: MagicMock,
                                   mock_zc_cls: MagicMock) -> None:
        mock_zc = MagicMock()
        mock_zc_cls.return_value = mock_zc

        # Simulate the listener getting called with a service
        def fake_browser(zc: Any, stype: str, listener: Any) -> MagicMock:
    ...
    @patch.object(setup, "ServiceBrowser", create=True)
    @patch("time.sleep")
    def test_no_duplicates(self, mock_sleep: MagicMock, mock_browser_cls: MagicMock, mock_zc_cls: MagicMock) -> None:
        mock_zc = MagicMock()
        mock_zc_cls.return_value = mock_zc
...
class TestGetCurrentSsid(unittest.TestCase):
    ...
    def test_linux_iwgetid(self, *_: Any) -> None:
        self.assertEqual(setup._get_current_ssid(), "Miele@home")

    @patch("platform.system", return_value="Linux")
    @patch("subprocess.check_output", return_value="\n")
    def test_linux_empty(self, *_: Any) -> None:
    ...
    @patch("platform.system", return_value="Darwin")
    @patch("subprocess.check_output")
    def test_macos_airport(self, mock_check: MagicMock, _: Any) -> None:
...
class TestDetectMieleApPassword(unittest.TestCase):
    ...
    def test_plain_ssid(self, _: Any) -> None:
    ...
    def test_suffixed_ssid(self, _: Any) -> None:
        pw, explanation = setup.detect_miele_ap_password()
...
class TestProvisionWifi(unittest.TestCase):
    @patch("requests.put")
    def test_http_success(self, mock_put: MagicMock) -> None:
        mock_resp = MagicMock()
    ...
    @patch("requests.put")
    def test_both_fail(self, mock_put: MagicMock) -> None:
        mock_put.side_effect = setup.requests.RequestException("timeout")
        result = setup.provision_wifi("192.168.1.1", "MyWifi", "pass123")
        self.assertFalse(result)

    @patch("requests.put")
    def test_http_fails_https_succeeds(self, mock_put: MagicMock) -> None:
        mock_fail = MagicMock()
    ...
    @patch("requests.put")
    def test_payload_format(self, mock_put: MagicMock) -> None:
        mock_resp = MagicMock()
        mock_resp.status_code = 200
        mock_resp.text = "OK"
        mock_put.return_value = mock_resp
    ...
    @patch("requests.put")
    def test_https_disables_cert_verify(self, mock_put: MagicMock) -> None:
        mock_resp = MagicMock()
        mock_resp.status_code = 200
        mock_resp.text = "OK"
        mock_put.return_value = mock_resp
...
class TestProvisionKeys(unittest.TestCase):
    @patch("time.sleep")
    @patch("requests.put")
    def test_http_succeeds_first_try(self, mock_put: MagicMock, mock_sleep: MagicMock) -> None:
    ...
    @patch("time.sleep")
    @patch("requests.put")
    def test_retries_on_failure(self, mock_put: MagicMock, mock_sleep: MagicMock) -> None:
    ...
    @patch("time.sleep")
    @patch("requests.put")
    def test_all_retries_fail(self, mock_put: MagicMock, mock_sleep: MagicMock) -> None:
        mock_put.side_effect = setup.requests.RequestException("timeout")
    ...
    @patch("time.sleep")
    @patch("requests.put")
    def test_http_400_retries_then_https_succeeds(self, mock_put: MagicMock, mock_sleep: MagicMock) -> None:
    ...
    @patch("time.sleep")
    @patch("requests.put")
    def test_https_uses_pairing_header(self, mock_put: MagicMock, mock_sleep: MagicMock) -> None:
    ...
    @patch("time.sleep")
    @patch("requests.put")
    def test_http_success_breaks_early(self, mock_put: MagicMock, mock_sleep: MagicMock) -> None:
        ok_resp = MagicMock(status_code=200, text="OK")
        mock_put.return_value = ok_resp
...
class TestProbeWifiPassword(unittest.TestCase):
    @patch("requests.get")
    def test_http_works(self, mock_get: MagicMock) -> None:
        mock_resp = MagicMock(status_code=200)
        mock_get.return_value = mock_resp
        self.assertEqual(setup.probe_wifi_password("10.0.0.1"), "http")

    @patch("requests.get")
    def test_https_fallback(self, mock_get: MagicMock) -> None:
    ...
    @patch("requests.get")
    def test_500_is_rejected(self, mock_get: MagicMock) -> None:
